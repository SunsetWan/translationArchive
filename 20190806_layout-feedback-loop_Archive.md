...
## 布局反馈循环
我们将要看一下布局反馈循环。它不是一个频繁出现的问题，但是一旦你遇到了它，它可能让你很头痛。
> 当你的视图正在运行它们的布局代码，但某种方法导致它们再一次开始布局传递，此时布局反馈循环就会出现。这可能是某个视图正在改变它某个父视图的大小的结果，或者可能因为你有一个模棱两可的布局。无论哪种方式，这个问题表现为你的 CPU 使用率最大化和 RAM 使用量稳步上升，所有这些是因为你的视图正再一次又一次地运行它们的布局代码，却没有返回。
> \- [Paul Hudson from HackingWithSwift](https://www.hackingwithswift.com/articles/59/debugging-auto-layout-feedback-loops)

幸运的是，在 WWDC 16 中 Apple 花了整整 15 分钟来介绍布局反馈循环调试器。这个调试器有助于识别在调试过程中发生循环的时刻。这就是一个符号断点，它的工作方式非常简单：它会计算在单个运行循环迭代中调用每个视图上的 `layoutSubviews()` 方法的次数。一旦这个计数值超过某个临界值（比如，100），这个应用程序将会停在这个断点并且打印出一个很好的日志，来帮助你找到根本原因。[这篇文章](https://www.hackingwithswift.com/articles/59/debugging-auto-layout-feedback-loops) 快速地介绍如何使用这个调试器。
